...
export function TV({ localConn, variant }: FurnitureProps) {
  ...
  const innerWidth =
    width - (localConn.right ? 0 : 0.02) - (localConn.left ? 0 : 0.02);
  const innerPosX =
    posX + (localConn.right ? 0.01 : 0) - (localConn.left ? 0.01 : 0);
  ...
  return (
    <group>
      ...
      {/* Screen Inner */}
      <Box
        args={[innerWidth, height - 0.04, 0.01]}
        position={[innerPosX, screenY, 0.026]}
      >
