...
export function Library({ localConn, variant }: FurnitureProps) {
  const libWidth =
    0.8 + (localConn.right ? 0.1 : 0) + (localConn.left ? 0.1 : 0);
  const libPosX = (localConn.right ? 0.05 : 0) - (localConn.left ? 0.05 : 0);

  const shelfWidth =
    0.7 + (localConn.right ? 0.15 : 0) + (localConn.left ? 0.15 : 0);
  ...
  return (
    <group position={[0, 0, -0.35]}>
      <Box
        args={[libWidth, 1.5, 0.3]}
        position={[libPosX, 0.75, 0]}
        castShadow
        receiveShadow
      >
